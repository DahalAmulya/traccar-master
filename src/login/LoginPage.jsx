...
const LoginPage = () => {
    ...
    if (nativeEnvironment) {
      ...
      try {
        ...
        const response = await fetch(
          "http://108.181.186.122:8082/api/session/token",
          {
            method: "POST",
  ...
  const handlePasswordLogin = async (event) => {
    ...
    try {
      ...
      const response = await fetch("http://108.181.186.122:8082/api/session", {
        method: "POST",
  ...
  const handleTokenLogin = useCatch(async (token) => {
    const response = await fetch(
      `http://108.181.186.122:8082/api/session?token=${encodeURIComponent(
        token
      )}`
    );
    if (response.ok) {
  ...
  const handleOpenIdLogin = () => {
    document.location = "http://108.181.186.122:8082/api/session/openid/auth";
  };
