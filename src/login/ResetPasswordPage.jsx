...
const ResetPasswordPage = () => {
  ...
  const handleSubmit = useCatch(async (event) => {
    ...
    if (!token) {
      response = await fetch("http://108.181.186.122:8082/api/password/reset", {
        method: "POST",
        body: new URLSearchParams(`email=${encodeURIComponent(email)}`),
      });
    } else {
      response = await fetch(
        "http://108.181.186.122:8082/api/password/update",
        {
          method: "POST",
    ...
    if (response.ok) {
      ...
      // Example: Update state based on response data
      if (responseData.success) {
        setSnackbarOpen(true);
      } else {
        // Handle other scenarios based on response data
      }
    } else {
