...
const SocketController = () => {
      ...
      if (event.code !== logoutCode) {
        try {
          const devicesResponse = await fetch(
            "108.181.186.122:8082/api/devices"
          );
          if (devicesResponse.ok) {
            dispatch(devicesActions.update(await devicesResponse.json()));
          }
          const positionsResponse = await fetch(
            "108.181.186.122:8082/api/positions"
          );
  ...
  useEffectAsync(async () => {
    if (authenticated) {
      try {
        const response = await fetch("http://108.181.186.122:8082/api/devices"); // Ensure to use the full URL with 'http://' or 'https://'

        if (response.ok) {
          const jsonData = await response.json(); // Parse response body as JSON

          if (jsonData) {
            dispatch(devicesActions.refresh(jsonData)); // Dispatch the JSON data to update Redux state
          } else {
        ...
        connectSocket(); // Connect to socket after successful fetch
        return () => {
