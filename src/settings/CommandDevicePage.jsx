  ...
  const handleSend = useCatch(async () => {
    ...
    if (savedId) {
      const response = await fetch(
        `http://108.181.186.122:8082/api/commands/${savedId}`
      );
      if (response.ok) {
    ...
    const response = await fetch(
      "http://108.181.186.122:8082/api/commands/send",
      {
        method: "POST",
        ...
        <Accordion defaultExpanded>
          ...
          <AccordionDetails className={classes.details}>
            <SelectField
              ...
              endpoint={`http://108.181.186.122:8082/api/commands/send?deviceId=${id}`}
