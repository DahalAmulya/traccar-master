...
const StatusCard = ({ deviceId, position, onClose, disableActions, desktopPadding = 0 }) => {
  ...
  const handleRemove = useCatch(async (removed) => {
    if (removed) {
      const response = await fetch("http://108.181.186.122:8082/api/devices");
      if (response.ok) {
  ...
  const handleGeofence = useCatchCallback(async () => {
    ...
    if (response.ok) {
      ...
      const permissionResponse = await fetch(
        "http://108.181.186.122:8082/api/permissions",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            ...
            <Card elevation={3} className={classes.card}>
              {deviceImage ? (
                <CardMedia
                  ...
                  image={`http://108.181.186.122:8082/api/media/${device.uniqueId}/${deviceImage}`}
                >
                  <IconButton
                    size="small"
                    onClick={onClose}
