...
Ext.define("Traccar.view.dialog.SendCommandController", {
  ...
  requires: ["Traccar.view.permissions.SavedCommands"],
  ...
  onSendClick: function (button) {
    ...
    Ext.Ajax.request({
      scope: this,
      url: "http://108.181.186.122:8082/api/commands/send",
  ...
  onValidityChange: function (form, valid) {
    this.lookupReference("sendButton").setDisabled(
      !valid || this.lookupReference("commandsComboBox").getValue() === null
    );
  },
  ...
  onCommandSelect: function (selected) {
    var record,
      form,
      command = selected.getStore().getById(selected.getValue());
    ...
    if (record && command.get("type") === record.get("type")) {
      this.onTypeChange(
        this.lookupReference("commandType"),
        command.get("type")
      );
    }
  ...
  onSendResult: function (options, success, response) {
    if (success) {
      ...
      Traccar.app.showToast(
        response.status === 202 ? Strings.commandQueued : Strings.commandSent
      );
    } else {
