...
Ext.define("Traccar.view.dialog.LoginController", {
  ...
  requires: ["Traccar.view.dialog.Register"],

  init: function () {
    this.lookupReference("resetButton").setHidden(
      !Traccar.app.getServer().get("emailEnabled")
    );
    ...
    Ext.Msg.prompt(Strings.loginReset, Strings.userEmail, function (btn, text) {
      if (btn === "ok") {
        Ext.Ajax.request({
          scope: this,
          method: "POST",
          url: "http://108.181.186.122:8082/api/password/reset",
          params: {
            email: text,
          },
          callback: function (options, success, response) {
