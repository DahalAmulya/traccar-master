...
Ext.define("Traccar.controller.Root", {
  ...
  calculateAttribute: function (data) {
    ...
    if (value !== undefined) {
      return Traccar.AttributeFormatter.getAttributeConverter(
        this.attributeKey
      )(value);
    }
    return value;
  ...
  onLaunch: function () {
    Ext.Ajax.request({
      scope: this,
      url: "http://108.181.186.122:8082/api/server",
  ...
  showAnnouncement: function (announcement) {
    var maxWidth =
      Ext.getBody().getViewSize().width - 2 * Traccar.Style.normalPadding;
  ...
  onServerReturn: function (options, success, response) {
    ...
    if (success) {
      ...
      Ext.Ajax.request({
        scope: this,
        url: "http://108.181.186.122:8082/api/session",
        method: "GET",
        params: parameters,
  ...
  onSessionReturn: function (options, success, response) {
    ...
    if (success) {
      ...
      passwordReset = Ext.Object.fromQueryString(
        window.location.search
      ).passwordReset;
      if (passwordReset) {
        dialog = Ext.Msg.prompt(
          Strings.loginReset,
          Strings.userPassword,
          function (btn, text) {
            ...
            if (btn === "ok") {
              Ext.Ajax.request({
                scope: this,
                method: "POST",
                url: "http://108.181.186.122:8082/api/password/update",
                params: {
                  token: passwordReset,
                  password: text,
                },
  ...
  loadApp: function () {
    ...
    Ext.getStore("ServerAttributes").loadData(
      Ext.getStore("CommonDeviceAttributes").getData().items,
      true
    );
    Ext.getStore("ServerAttributes").loadData(
      Ext.getStore("CommonUserAttributes").getData().items,
      true
    );
    Ext.getStore("UserAttributes").loadData(
      Ext.getStore("CommonUserAttributes").getData().items,
      true
    );
    Ext.getStore("DeviceAttributes").loadData(
      Ext.getStore("CommonDeviceAttributes").getData().items,
      true
    );
    Ext.getStore("GroupAttributes").loadData(
      Ext.getStore("CommonDeviceAttributes").getData().items,
      true
    );
  ...
  removeUrlParameter: function (param) {
    ...
    if (Ext.Object.isEmpty(params)) {
      ...
      window.history.pushState(
        null,
        null,
        window.location.pathname + "?" + Ext.Object.toQueryString(params)
      );
    }
  },
  ...
  asyncUpdate: function (first) {
    ...
    socket = new WebSocket(
      protocol +
        ...
        pathname +
        "http://108.181.186.122:8082/api/socket"
    );

    socket.onclose = function () {
      Ext.Ajax.request({
        url: "http://108.181.186.122:8082/api/devices",
      ...
      Ext.Ajax.request({
        url: "http://108.181.186.122:8082/api/positions",
        headers: {
  ...
  updatePositions: function (array, first) {
    ...
    for (i = 0; i < array.length; i++) {
      ...
      if (
        Ext.getStore("Events").findRecord(
          "positionId",
          array[i].id,
          0,
          false,
        ...
        Ext.getStore("EventPositions").add(
          Ext.create("Traccar.model.Position", array[i])
        );
      }
    }
    if (first) {
      ...
      if (uniqueId) {
        device = Ext.getStore("VisibleDevices").findRecord(
          "uniqueId",
          uniqueId,
          0,
          false,
